...
pub fn parse_literal(size: InstrSize, mc: &[u8]) -> IResult<&[u8], Token> {
    ...
    match size {
        ...
        InstrSize::I8 => map(tuple((tag(&[LITERAL]), take(1_usize))), |res| Token::I8(u8::from_bytes(res.1).unwrap()))(mc),
        InstrSize::I16 => map(tuple((tag(&[LITERAL]), take(2_usize))), |res| Token::I16(u16::from_bytes(res.1).unwrap()))(mc),
        InstrSize::I32 => map(tuple((tag(&[LITERAL]), take(4_usize))), |res| Token::I32(u32::from_bytes(res.1).unwrap()))(mc),
        InstrSize::F32 => map(tuple((tag(&[LITERAL]), take(4_usize))), |res| Token::F32(f32::from_bytes(res.1).unwrap()))(mc),
        InstrSize::I64 => map(tuple((tag(&[LITERAL]), take(8_usize))), |res| Token::I64(u64::from_bytes(res.1).unwrap()))(mc),
        InstrSize::F64 => map(tuple((tag(&[LITERAL]), take(8_usize))), |res| Token::F64(f64::from_bytes(res.1).unwrap()))(mc),
        InstrSize::I128 => map(tuple((tag(&[LITERAL]), take(16_usize))), |res| Token::I128(u128::from_bytes(res.1).unwrap()))(mc),
    }
